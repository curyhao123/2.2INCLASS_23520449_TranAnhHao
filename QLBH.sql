...
--Cau 7:
Select SOHD,TRIGIA from HOADON where NGHD >= '2007-1-1' and NGHD <'2007-2-1' 
ORDER BY NGHD ASC, TRIGIA DESC;
--Cau 8:
SELECT MAKH, HOTEN
FROM KHACHHANG
WHERE MAKH IN (SELECT MAKH FROM HOADON WHERE NGHD = '2007-01-01');
--Cau 9:
Select SOHD,TRIGIA from HOADON where NGHD='2006-10-28' AND MANV IN (select MANV from NHANVIEN where HOTEN ='Nguyen Van B');
--Cau 10:
Select MASP,TENSP from SANPHAM 
where MASP IN (select MASP from CTHD where SOHD IN 
(select SOHD from HOADON where MAKH IN 
(select MAKH from KHACHHANG where HOTEN='Nguyen Van A') AND (NGHD >='2006-10-1'AND NGHD<  '2006-11-1')));
--Cau 11:
Select DISTINCT HOADON.SOHD from HOADON, CTHD where 
HOADON.SOHD=CTHD.SOHD AND
(CTHD.MASP ='BB01' OR CTHD.MASP='BB02');
